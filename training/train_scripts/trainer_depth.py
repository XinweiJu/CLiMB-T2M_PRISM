...
class Trainer:
    def __init__(self, options):
        ...
        #     "encoder": "/Datasets/Checkpoints/monodepth/mono_640x192/encoder.pth",
        #     "depth": "/Datasets/Checkpoints/monodepth/mono_640x192/depth.pth",
        #     "pose_encoder": "/Datasets/Weights_Feast_new/monodepth/hk_mono_finetuned_edge_ssim_3/models/weights_29/pose_encoder.pth",
        #     "pose": "/Datasets/Weights_Feast_new/monodepth/hk_mono_finetuned_edge_ssim_3/models/weights_29/pose.pth",
        # }
        ...
        # Freeze depth network parameters for pose-only training
        self.freeze_depth_network()

    def freeze_depth_network(self):
        """Freeze all depth-related parameters"""
        print("🔒 Freezing depth network parameters...")
        ...
        if "predictive_mask" in self.models:
            for param in self.models["predictive_mask"].parameters():
                param.requires_grad = False
        print("✅ Depth network frozen. Only pose network will be trained.")
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            # ---- Step 1: backward for depth (FROZEN - only for pose training)
            # Skip depth optimizer step to freeze depth parameters
            ...
            # ---- Step 3: optimizer step (only pose)
            # Skip depth optimizer step to freeze depth parameters
            ...
            if early_phase or late_phase:
                self.log_time(batch_idx, duration, losses["loss_depth"].cpu().data)
        ...
        # Skip depth scheduler since depth is frozen
    ...
    def compute_losses(self, inputs, outputs):
        ...
        losses["loss_pose"] = total_loss + total_loss_edge if self.opt.edge_loss else total_loss
