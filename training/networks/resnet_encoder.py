...
class ResnetEncoder(nn.Module):
    ...
    Supports the RGB plus auxiliary channel layouts used during PRISM training.
    """
    ...
    def __init__(
        ...
        # Every CLiMB run immediately loads a complete PRISM checkpoint. Avoid
        # torchvision and its network download even when the legacy option says
        # "pretrained".
        self.num_ch_enc = np.array([64, 64, 128, 256, 512])
        self.encoder = ResNet18(
            input_channels=channels_per_image * num_input_images
        )
