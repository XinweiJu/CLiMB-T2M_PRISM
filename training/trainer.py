...
try:
    ...
    class SummaryWriter:
        """No-op fallback; checkpoints still work without TensorBoard installed."""
...
import glob
def train_test_split(items, test_size, shuffle=False):
    """Small sequential split used only by the legacy automatic split branch."""
    if shuffle:
        raise ValueError("This standalone trainer only supports sequential splits")
    count = max(1, int(round(len(items) * test_size)))
...
class Trainer:
    def __init__(self, options):
        ...
        motion_channels = 1 if (self.opt.motion_aux != "none" and self.opt.motion_fusion == "channel") else 0
        self.channels_per_image_depth = (4 if self.opt.depth_aux != "none" else 3) + motion_channels
        self.channels_per_image_pose = (4 if self.opt.pose_aux != "none" else 3) + motion_channels
        ...
        self.depth_use_lum = self.opt.depth_aux == "lum"
        self.depth_use_edge = self.opt.depth_aux == "edge"
        self.pose_use_edge = self.opt.pose_aux == "edge"
        self.pose_use_lum = self.opt.pose_aux == "lum"
        ...
        if self.opt.freeze_depth:
            for name in ["encoder", "depth", "predictive_mask"]:
                if name in self.models:
                    for parameter in self.models[name].parameters():
                        parameter.requires_grad = False
        if self.opt.freeze_pose:
            for name in ["pose_encoder", "pose"]:
                if name in self.models:
                    for parameter in self.models[name].parameters():
                        parameter.requires_grad = False
        self.parameters_to_train = [
        ...
        if self.opt.dataset == "hk":
            train_dataset = self.dataset(
                ...
                data=self.opt.data, distorted=self.opt.distorted,
                motion_aux=self.opt.motion_aux, motion_root=self.opt.motion_root,
                shading_root=self.opt.shading_root, edge_root=self.opt.edge_root)
        else:
            train_dataset = self.dataset(
                self.opt.data_path, train_filenames, self.opt.height, self.opt.width,
                self.opt.frame_ids, 4, is_train=True, img_ext=img_ext)
    ...
    def set_train(self):
        ...
        for name, model in self.models.items():
            frozen = (
                self.opt.freeze_depth
                and name in ["encoder", "depth", "predictive_mask"]
            ) or (
                self.opt.freeze_pose
                and name in ["pose_encoder", "pose"]
            )
    ...
    def run_epoch(self):
        ...
        if self.opt.benchmark_batches and self.device.type == "cuda":
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            if (early_phase or late_phase) and not self.opt.benchmark_batches:
            ...
            self.step += 1
            if (
                self.opt.benchmark_batches
                and batch_idx + 1 >= self.opt.benchmark_batches
            ):
                ...
                print(
                    "BENCHMARK batches={} batch_size={} seconds={:.3f} "
                    "seconds_per_batch={:.4f} samples_per_second={:.2f} "
                    "peak_memory_gib={:.3f}".format(
                        batch_idx + 1,
                        self.opt.batch_size,
                        elapsed,
                        elapsed / (batch_idx + 1),
                        (batch_idx + 1) * self.opt.batch_size / elapsed,
                        peak_gib,
                    ),
                    flush=True,
                )
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            if self.depth_use_lum:
                inputs_feature = torch.cat([inputs[("color_aug", 0, 0)], inputs[("lum", 0, 0)]], dim=1)
            elif self.depth_use_edge:
                inputs_feature = torch.cat([inputs[("color_aug", 0, 0)], inputs[("edge", 0, 0)]], dim=1)
            else:
                inputs_feature = inputs[("color_aug", 0, 0)]
            if self.opt.motion_aux != "none":
                motion = inputs[("motion", 0, 0)]
                if self.opt.motion_fusion == "channel":
                    inputs_feature = torch.cat([inputs_feature, motion], dim=1)
                else:
                    auxiliary = inputs_feature[:, -1:]
                    if self.opt.motion_fusion == "add":
                        auxiliary = torch.clamp(auxiliary + self.opt.motion_alpha * motion * 255.0, 0.0, 255.0)
                    else:
                        auxiliary = (1.0 - self.opt.motion_alpha) * auxiliary + self.opt.motion_alpha * motion * 255.0
                    inputs_feature = torch.cat([inputs_feature[:, :-1], auxiliary], dim=1)
    ...
    def predict_poses(self, inputs, features):
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type == "shared":
                ...
                if self.pose_use_edge or self.pose_use_lum:
                    # if self.pose_use_edge == True and self.pose_use_lum == True:
                    #     pose_feats = {
                    #         f_i: torch.cat([inputs[("color_aug", f_i, 0)], inputs[("edge", f_i, 0)], inputs[("lum", f_i, 0)]], dim=1)
                    #         for f_i in self.opt.frame_ids
                    #     }
                    # el
                    if self.pose_use_edge:
                        pose_feats = {
                            f_i: torch.cat([inputs[("color_aug", f_i, 0)], inputs[("edge", f_i, 0)]], dim=1)
                            for f_i in self.opt.frame_ids
                        }
                    elif self.pose_use_lum:
                        pose_feats = {
                ...
                if self.opt.motion_aux != "none":
                    if self.opt.motion_fusion == "channel":
                        pose_feats = {
                            f_i: torch.cat([pose_feats[f_i], inputs[("motion", f_i, 0)]], dim=1)
                            for f_i in self.opt.frame_ids if f_i != "s"
                        }
                    else:
                        fused_pose_feats = {}
                        for f_i in self.opt.frame_ids:
                            if f_i == "s":
                                continue
                            feat = pose_feats[f_i]
                            motion = inputs[("motion", f_i, 0)]
                            auxiliary = feat[:, -1:]
                            if self.opt.motion_fusion == "add":
                                auxiliary = torch.clamp(auxiliary + self.opt.motion_alpha * motion, 0.0, 1.0)
                            else:
                                auxiliary = (1.0 - self.opt.motion_alpha) * auxiliary + self.opt.motion_alpha * motion
                            fused_pose_feats[f_i] = torch.cat([feat[:, :-1], auxiliary], dim=1)
                        pose_feats = fused_pose_feats
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                if self.opt.edge_loss:
                    ...
                    if self.edge_loss_type == "edge_dice":
                        ...
                        reprojection_losses.append(0.5 * reprojection_edge.float())
                ...
                for frame_id in self.opt.frame_ids[1:]:
                    ...
                    if self.opt.edge_loss:
                        ...
                        if self.edge_loss_type == "edge_dice":
                            ...
                            identity_reprojection_losses.append(0.5 * reprojection_edge_check.float())
    ...
    def load_model(self):
        ...
        for n in self.opt.models_to_load:
            ...
            for k, v in pretrained_dict.items():
                if k in model_dict and isinstance(v, torch.Tensor) and v.shape == model_dict[k].shape:
                    ...
                    if channels_per_image < pretrained_channels_per_image:
                        raise ValueError(
                            "Cannot shrink conv1 from {} to {} channels/image".format(
                                pretrained_channels_per_image, channels_per_image))

                    source = v.reshape(
                        v.shape[0], num_input_images,
                        pretrained_channels_per_image, *v.shape[2:])
                    blocks = []
                    for image_index in range(num_input_images):
                        block = source[:, image_index]
                        missing = channels_per_image - pretrained_channels_per_image
                        if missing > 0:
                            # Preserve every original RGB+auxiliary channel exactly.
                            # Start new motion channels near zero using 0.1 times
                            # the original auxiliary channel (or RGB mean).
                            seed = (
                                block[:, 3:4]
                                if pretrained_channels_per_image > 3
                                else block.mean(dim=1, keepdim=True)
                            )
                            block = torch.cat(
                                [block, seed.repeat(1, missing, 1, 1) * 0.1], dim=1)
                        blocks.append(block)
                    new_conv1 = torch.cat(blocks, dim=1)
        ...
        if self.opt.load_optimizer and os.path.isfile(optimizer_load_path):
            ...
            print("Adam is freshly initialized")
